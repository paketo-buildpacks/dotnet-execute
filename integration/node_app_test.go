...
func testNodeApp(t *testing.T, context spec.G, it spec.S) {
	...
	context("when building a node app", func() {
		var (
			image     occam.Image
		...
		it("builds and runs successfully", func() {
			...
			// check an SBOM file
			contents, err := os.ReadFile(filepath.Join(sbomDir, "sbom", "launch", strings.ReplaceAll(settings.BuildpackInfo.Buildpack.ID, "/", "_"), "sbom", "sbom.cdx.json"))
			Expect(err).NotTo(HaveOccurred())
