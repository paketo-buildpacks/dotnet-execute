...
func testBuild(t *testing.T, context spec.G, it spec.S) {
	...
	context("the app is a framework-dependent or self-contained executable", func() {
		...
		it("returns a result that builds correctly", func() {
			...
			// The result layer path is autogenerated, so exact equality will fail
			Expect(result.Layers[0].Path).To(MatchRegexp(".*port-chooser$"))
			portChooserLayer.Path = result.Layers[0].Path

			Expect(result).To(Equal(packit.BuildResult{
				Plan: packit.BuildpackPlan{
					Entries: nil,
				},
				Layers: []packit.Layer{portChooserLayer},
